...
export function ChatPanel({ context, placeholder = '건강보험 청구, 심사기준 등 업무 관련 질문을 입력하세요...' }: ChatPanelProps) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((prev) => {
        const updated = [...prev]
        updated[updated.length - 1] = { role: 'assistant', content: '오류가 발생했습니다. 다시 시도해 주세요.' }
        return updated
      })
    } finally {
      setLoading(false)
